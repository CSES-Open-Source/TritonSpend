...
export default function GoalsRow(props: any){
    ...
    return(
        ...
        <Animated.View style={[styles.GoalsContainer, {height: expand, backgroundColor: props.color}]} >
            <Pressable onPress={toggle}  onLongPress={() => setModalVisible(true)} style = {{height:'100%', width:'80%'}}>
                <Text style = {styles.title}>{props.title}</Text>
                <Text>{props.date}</Text>
                {inputVisible? (
                    <Animated.View style={[styles.content, {transform: [{translateY: inputShow}], opacity: animatedOpacity}]}>
